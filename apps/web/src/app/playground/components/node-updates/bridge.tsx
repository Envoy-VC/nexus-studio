...
import type { BridgeNodeData, NodeMetadata } from "@/types";

interface Props {
  node: Node<BridgeNodeData>;
  metadata: NodeMetadata;
}

export const BridgeNodeUpdate = ({ node }: Props) => {
  const { updateNodeData } = useReactFlowStore();

  const sourceChains = useMemo(() => {
    return node.data.sourceChains?.map((chain) => chain.toString()) ?? [];
  }, [node]);

  return (
    <div className="flex flex-col gap-2">
      <div className="px-2 font-bold text-lg">Bridge Details</div>
